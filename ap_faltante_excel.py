...
def init_db():
    ...
    for col, typ in [
        ...
        # message_status (agregar origin)
    _try_alter(cur, "ALTER TABLE message_status ADD COLUMN origin TEXT;")
    _try_alter(cur, "CREATE INDEX IF NOT EXISTS idx_msg_origin ON message_status(origin);")
...
def save_template_sid(tenant: str, cuil: str, period: str, to_whatsapp: str, sid: str, nombre: str = "", origin: str = "INITIAL"):
    now = int(time.time())
    conn = get_db_connection()
    cur = conn.cursor()
    cur.execute("""
        INSERT OR IGNORE INTO message_status
        (message_sid, to_whatsapp, tenant, cuil, period, nombre, kind, origin, created_at, last_status, last_status_at)
        VALUES (?, ?, ?, ?, ?, ?, 'template', ?, ?, 'sent', ?)
    """, (sid, to_whatsapp, tenant, cuil, period, nombre, origin, now, now))
    conn.commit()
    conn.close()
...
def generate_excel_report_v3(tenant: str, period_filter: str = "", mode: str = "ALL") -> BytesIO:
    tenant = (tenant or "").strip().lower()
    period_filter = norm_period_label(period_filter)
    ...
    cur.execute("""
        SELECT
            ms.to_whatsapp, ms.tenant, ms.cuil, ms.period, ms.nombre, ms.kind,
            ms.created_at, ms.delivered_at, ms.read_at, ms.failed_at,
            sp.origin AS pdf_origin
        FROM message_status ms
        LEFT JOIN sent_pdfs sp
          ON sp.message_sid = ms.message_sid
        WHERE ms.tenant = ?
    """, (tenant,))
    msg_rows = cur.fetchall()
    ...
    cur.execute("""
        SELECT to_whatsapp, MAX(created_at) as last_ts
        FROM pending_views
        WHERE tenant = ?
        GROUP BY to_whatsapp
    """, (tenant,))
    ...
    for r in pv_rows:
        ...
        if not w:
            continue
        # Traer el period real del último pending_views de ese whatsapp:
        ...
        cur2.close()
        if rr:
            p = norm_period_label(rr["period"] or "")
            if p:
                last_period_by_whatsapp[w] = p
    ...
    for row in msg_rows:
        ...
        period_norm = norm_period_label((row["period"] or "").strip())
        if not period_norm:
            period_norm = last_period_by_whatsapp.get(wpp, "")
    ...
    filtered = []
    for rec in agg.values():
        has_template = bool(rec.get("plantilla_sent_at"))
        origin = (rec.get("pdf_origin") or "").strip()
        pedidos = int(rec.get("pedidos_recibo") or 0)

        if mode == "ADMIN":
            if has_template or origin == "INITIAL":
                filtered.append(rec)
        elif mode == "RESENDS":
            if (origin and origin != "INITIAL") or pedidos > 0:
                filtered.append(rec)
        else:
            filtered.append(rec)
    ...
    filtered.sort(key=lambda r: (r.get("periodo") or "", r.get("nombre") or "", r.get("whatsapp") or ""))
    ...
    buf.seek(0)
    return buf
@app.get("/admin/report_envios.xlsx")
def admin_report_envios_xlsx():
    token = _get_admin_token_from_request()
    tenant = (request.args.get("tenant") or "").strip().lower()
    period = (request.args.get("period") or "").strip()
    ...
    buf = generate_excel_report_v3(tenant, period_filter=period, mode="ADMIN")

    filename = f"reporte_envios_admin_{tenant}_{(norm_period_label(period).replace('/','-') if period else 'todos')}.xlsx"
    return send_file(
        buf,
        as_attachment=True,
        download_name=filename,
...
@app.get("/admin/report_envios_admin.xlsx")
def admin_report_envios_admin_xlsx():
    token = _get_admin_token_from_request()
    tenant = (request.args.get("tenant") or "").strip().lower()
    period = (request.args.get("period") or "").strip()
    ...
    buf = generate_excel_report_v3(tenant, period_filter=period, origin_filter="INITIAL")
...
@app.get("/admin/report_reenvios.xlsx")
def admin_report_reenvios_xlsx():
    token = _get_admin_token_from_request()
    tenant = (request.args.get("tenant") or "").strip().lower()
    period = (request.args.get("period") or "").strip()
    ...
    filename = f"reporte_reenvios_{tenant}_{(norm_period_label(period).replace('/','-') if period else 'todos')}.xlsx"
    return send_file(
        buf,
        as_attachment=True,
        download_name=filename,
...
""")
    ...
    # ✅ XLSX completo (mezcla todo)
    html.append(
        f"<a class='btn secondary' href='/admin/report_recibos.xlsx?tenant={esc(tenant)}&period={period_q}&token={esc(token)}'>📄 Reporte completo (XLSX)</a>"
    )

    # ✅ XLSX solo envíos del admin
    html.append(
        f"<a class='btn' href='/admin/report_envios_admin.xlsx?tenant={esc(tenant)}&period={period_q}&token={esc(token)}'>📤 Envíos admin (XLSX)</a>"
    )

    # ✅ XLSX solo reenvíos / pedidos
    html.append(
        f"<a class='btn' href='/admin/report_reenvios.xlsx?tenant={esc(tenant)}&period={period_q}&token={esc(token)}'>🔁 Reenvíos (XLSX)</a>"
    )

    # PDF y CSV como estaban
    html.append(
...
def admin_send_template_queue_tick():
    ...
    for r in rows:
        ...
        try:
            ...
            # 4) Persistir
            save_template_sid(tenant, cuil, period, to_whatsapp, sid, nombre=nombre, origin="INITIAL") 

...
def admin_community_links():
    ...
    num = re.sub(r"\D", "", TWILIO_WHATSAPP_FROM)
    ...
    for t in load_tenants():
        ...
        aviso = (f"\U0001F4C4 Ya est\u00e1 disponible tu recibo de {period_legible}.\n"
                 ...
                 f"(La primera vez te va a pedir aceptar los T&C.)")
        html.append(f"<h3>{esc(disp)}</h3><p><a href='{link}'>{esc(link)}</a></p>"
                    f"<pre style='background:#f4f4f4;padding:1em;white-space:pre-wrap'>{esc(aviso)}</pre>")
    html.append("</body></html>")
    return Response("".join(html), mimetype="text/html")
...
def twiml(msg: str):
    return Response(
        f"<Response><Message>{_xml_escape(msg)}</Message></Response>",
...
_COMMUNITY_REJECT_MSG = (
    "\U0001F44B Este es el canal de entrega de recibos de sueldo para el personal de "
...
def handle_community_request(from_whatsapp: str, body: str):
    ...
    None y el inbound sigue su flujo normal.
    """
